...
func _physics_process(_delta):
	if dying and not $bricks.emitting and not $Tween.is_active():
		queue_free()
		...
		if Global.color_rotate >= 0:
			$brick.modulate = colors[int(floor(color_distance + Global.color_rotate)) % len(colors)]
...
func die():
	...
	$Tween.interpolate_property($brick, "modulate:a", $brick.modulate.a, 0, time_a, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.interpolate_property($brick, "modulate:s", $brick.modulate.s, 0, time_s, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.interpolate_property($brick, "modulate:v", $brick.modulate.v, 0, time_v, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
	$Tween.start()
